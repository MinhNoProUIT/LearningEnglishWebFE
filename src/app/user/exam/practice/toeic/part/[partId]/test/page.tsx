...
import { useGradeWritingMutation } from "@/services/WritingService";
...
const PART_INFO: Record<number, {
  ...
  8: {
    ...
    icon: <PenTool size={24} />,
    skillType: "WRITING",
    instructions: "In this section of the test, you will write based on the instructions provided for each question.",
  },
};

// ================== API DATA TRANSFORMERS ==================
const transformPracticeToQuestions = (data: any) => {
    ...
    try {
        ...
        const questionGroups = (data as any).question_groups || 
                              (data as any).data?.question_groups || 
                              (data as any).exam_section?.question_groups ||
                              (data as any).data?.exam_section?.question_groups ||
                              (data as any).exam_sections?.question_groups;

        if (!questionGroups || !Array.isArray(questionGroups)) {
             console.warn("Could not find question_groups in response:", data);
             return [];
        }

        // Helper to detect if a URL is likely an image
        const isImageUrl = (url?: string) => {
            if (!url) return false;
            return /\.(jpeg|jpg|gif|png|webp|svg|bmp)$/i.test(url) || url.includes("/image/upload");
        };

        questionGroups.forEach((group: any) => {
          // Safer safety check
          const groupTitle = group.group_title || "";
          const isPart1 = groupTitle.toLowerCase().includes("part 1");

          if (group.questions.length > 1) {
            // Correct logic for Part 1 "Grouped" questions with images in audio_url
            const subQuestions = group.questions.map((q: any, idx: number) => {
                // Determine separate Media URLs
                let finalImageUrl = q.metadata?.image_url || q.media_url;
                let finalAudioUrl = q.audio_url;

                // FIX: User reported Part 1 images are in audio_url
                if (isPart1 && !finalImageUrl && finalAudioUrl && isImageUrl(finalAudioUrl)) {
                    finalImageUrl = finalAudioUrl;
                    finalAudioUrl = undefined; // It's an image, not audio
                }
                // Generic check for mixed up data
                else if (!finalImageUrl && finalAudioUrl && isImageUrl(finalAudioUrl)) {
                     finalImageUrl = finalAudioUrl;
                ...
                return {
                  id: Number(q.id),
                  displayNo: q.order_index || (displayNo + idx),
                  type: q.question_type?.toLowerCase() || "multiple_choice",
            ...
            let finalImageUrl = group.media_type === "IMAGE" ? group.media_url : undefined;
            // Also check q.audio_url for image in Single case (Part 1 Single)
            if (isPart1 && !finalImageUrl && q.audio_url && isImageUrl(q.audio_url)) {
                finalImageUrl = q.audio_url;
            }

            questions.push({
              id: Number(q.id),
              displayNo: displayNo, 
              type: q.question_type?.toLowerCase() || (group.media_type === "IMAGE" ? "photograph" : "multiple_choice"),
              passage: group.content_text, 
        ...
        return questions.sort((a, b) => {
            return (a.displayNo || 0) - (b.displayNo || 0);
        }); 

    } catch (err) {
        console.error("Error transforming practice data:", err);
        return [];
    }
  };
...
// Updated playFullAudio to support multi-voice conversations
const playFullAudio = (
  text: string, 
  options: any[], 
  extraContent: { conversation?: string; readOptions?: boolean } = {}
) => {
    const { conversation = "", readOptions = true } = extraContent;
    ...
    ) || englishVoices.find(v => v.lang.startsWith("en-US")) || englishVoices[0];
    ...
    ) || englishVoices.find(v => v.lang.startsWith("en-GB")) || englishVoices[0];

    // Execution Queue
    let speechQueue: { text: string; voice: SpeechSynthesisVoice | null; pitch: number; rate: number }[] = [];

    // 1. Queue Conversation (if any)
    if (conversation) {
        const turns = parseConversation(conversation);
        turns.forEach(turn => {
            speechQueue.push({
                text: turn.text,
                voice: turn.speaker === 'male' ? maleVoice : femaleVoice,
                pitch: turn.speaker === 'male' ? 0.9 : 1.1,
                rate: 1.0
            });
        });
    }
    ...
    if (text) {
        speechQueue.push({
            text: text,
            voice: maleVoice, // Narrator usually male or neutral
            pitch: 1.0,
            rate: 1.0
        });
    }
    ...
    // 3. Queue Options
    if (readOptions && options && options.length > 0) {
        const optionsText = options.map((opt: any) => 
            `${opt.label}${SHORT_PAUSE}${opt.text}`
        ).join(LONG_PAUSE);
    ...
    const playNext = () => {
        ...
        utterance.onend = () => {
            currentIndex++;
            setTimeout(playNext, 300); // Small pause between turns
        };
            ...
            >
              <Box
                ...
                sx={{
                  ...
                  transition: "all 0.2s",
                  bgcolor: isAnswered
                    ? "#10b981" // Stronger green for answered, ONLY when answered
                    : isCurrent
                    ? "#ebf8ff" // Light blue for current but not answered
                    : "#f3f4f6", // Grey for unvisited
                  color: isAnswered
                    ? "white"
                    : isCurrent
...
export default function ToeicPartPracticePage() {
  ...
  // Get sectionId from URL query params, fallback to partId
  const sectionId = searchParams.get("sectionId") || String(partId);
  ...
  const { data: inProgressData, isLoading: isCheckingProgress } = useGetInProgressPracticeQuery(sectionId, {
    skip: !sectionId,
  });
  ...
  useEffect(() => {
    const initPractice = async () => {
      ...
      if (inProgressData) {
        ...
        if (inProgressData.saved_answers) {
          ...
          inProgressData.saved_answers.forEach((ans) => {
            if (ans.selected_option_id) {
              ...
              savedAnswersMap.set(ans.question_id, ans.text_answer);
            }
          });
        ...
        try {
          const result = await startPractice({ sectionId, partNumber: partId } as any).unwrap();
        ...
        // Start new practice
        try {
          console.log("Starting practice with sectionId:", sectionId);
          const result = await startPractice({ sectionId, partNumber: partId } as any).unwrap();
  ...
  }, [sectionId, isCheckingProgress, inProgressData, startPractice]);
  ...
  useEffect(() => {
    ...
    const saveTimer = setTimeout(async () => {
      const answersArray: IUserAnswer[] = Array.from(answers.entries()).map(
        ([questionId, value]) => ({
          questionId,
          selectedOptionId: typeof value === "number" ? value : undefined,
  ...
  // Handle submit
  const [gradeWriting] = useGradeWritingMutation();
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      // Special handling for Writing (Part 8)
      if (partInfo.skillType === "WRITING") {
         console.log("Grading writing answers...");
         const gradingPromises = answersArray.map(async (ans) => {
            if (ans.textAnswer) {
              return gradeWriting({
                sectionId: sectionId, // from URL param or derived
                questionId: ans.questionId,
                answerText: ans.textAnswer,
                level: "Intermediate" // Can be dynamic if user profile is available
              }).unwrap();
            }
         });
      ...
      // Submit practice (marks attempt as completed)
  ...
  }, [practiceId, answers, savePracticeProgress, submitPractice, router, partId, partInfo, sectionId, gradeWriting]);
  ...
  // Loading state
  if (isStarting || isCheckingProgress) {
    return (
      <Box
        sx={{
          minHeight: "100vh",
  ...
  // No Questions state (prevent infinite loading)
    ...
    return (
      ...
      >
        <Paper sx={{ p: 4, borderRadius: 3, textAlign: "center" }}>
          ...
          <Typography color="text.secondary" mb={3}>
            Không tìm thấy câu hỏi nào. Vui lòng kiểm tra lại.
          </Typography>
          <Button
            variant="contained"
            onClick={() => router.push("/user/exam")}
  ...
  // Waiting for first question render
  if (!currentQuestion) {
     return null; // Should be handled above, but safe fallback
  }
            ...
            >
              ...
              >
                <Stack direction="row" alignItems="center" spacing={2}>
                  <Chip
                    label={`Câu ${
                      currentQuestion.subQuestions 
                        ? `${currentQuestion.subQuestions[0].displayNo}-${currentQuestion.subQuestions[currentQuestion.subQuestions.length - 1].displayNo}`
                        : currentQuestion.displayNo
                    }`}
                    sx={{
                      background: theme.gradients.primary,
                      color: "white",
                  ...
                  />
                  {(partId === 3 || partId === 4) && (currentQuestion.conversationText || currentQuestion.passage) && (
                     <IconButton 
                        size="small" 
                        onClick={() => playFullAudio("", [], { conversation: currentQuestion.conversationText || currentQuestion.passage, readOptions: false })}
                  ...
                  <Chip
                    label={partInfo.skillType === "LISTENING" ? "Listening" : partInfo.skillType === "READING" ? "Reading" : "Writing"}
                    size="small"
                    icon={partInfo.skillType === "LISTENING" ? <Headphones size={14} /> : partInfo.skillType === "READING" ? <BookMarked size={14} /> : <PenTool size={14} />}
                    sx={{
                      bgcolor: partInfo.skillType === "LISTENING" ? "#fef3c7" : partInfo.skillType === "READING" ? "#dbeafe" : "#f1f5f9",
                      color: partInfo.skillType === "LISTENING" ? "#92400e" : partInfo.skillType === "READING" ? "#1e40af" : "#475569",
                      fontWeight: 600,
                    }}
                  />
                </Stack>
              ...
              {/* Passage for Reading/Writing */}
              {currentQuestion.passage && (partInfo.skillType === "READING" || partInfo.skillType === "WRITING") && (
                <Paper
                  elevation={0}
                  sx={{
                    p: 3,
              ...
              {currentQuestion.subQuestions ? (
                // Grouped questions (Part 3, 4, 6, 7)
                <Stack spacing={4}>
                  {currentQuestion.subQuestions.map((subQ, idx) => (
                    <Box key={subQ.id}>
                      {/* Image for SubQuestion (Part 1 Grouped) */}
                      {subQ.imageUrl && (
                        <Box
                          sx={{
                            mb: 2,
                        ...
                        >
                          <img
                            src={subQ.imageUrl}
                            alt={`Question ${subQ.displayNo} image`}
                            style={{
                              width: "100%",
                              maxHeight: 300,
                              objectFit: "contain",
                      ...
                      {subQ.questionText && (
                         <Typography
                           ...
                           mb={2}
                         >
                           {subQ.displayNo}. {partId === 2 ? "Nghe câu hỏi" : subQ.questionText.replace(/^\d+\.\s*/, '')}
                           {partInfo.skillType === "LISTENING" && (
                             <IconButton size="small" onClick={() => playFullAudio(subQ.questionText, [], { conversation: "", readOptions: false })} sx={{ ml: 1 }}>
                               <Volume2 size={16} color={theme.colors.primary} />
                             </IconButton>
                           )}
                           {/* Individual Audio for SubQuestion */}
                           {subQ.audioUrl && (
                              <IconButton size="small" onClick={() => new Audio(subQ.audioUrl).play()} sx={{ ml: 1 }}>
                                <Volume2 size={16} color={theme.colors.primaryDark} />
                              </IconButton>
                           )}
                         </Typography>
                      )}
                      
                      {subQ.type === "writing" || subQ.type === "essay" ? (
                        <TextField
                          fullWidth
                          multiline
                          rows={subQ.type === "essay" ? 12 : 5}
                                  ...
                                  label={
                                    <Stack direction="row" alignItems="center" spacing={1}>
                                      <Typography fontWeight={500}>
                                        <strong>{option.label}.</strong> {(partId !== 1 && partId !== 2) && option.text}
                                      </Typography>
                                    </Stack>
                                  }
                                  sx={{ m: 0, width: "100%" }}
                ...
                <Box>
                  {currentQuestion.questionText && (
                    <Typography
                      ...
                      mb={2}
                    >
                      {partId === 2 ? "Nghe câu hỏi" : currentQuestion.questionText}
                      {partInfo.skillType === "LISTENING" && (
                        <IconButton size="small" onClick={() => playFullAudio(currentQuestion.questionText || "", currentQuestion.options || [])} sx={{ ml: 1 }}>
                            <Volume2 size={16} color={theme.colors.primary} />
                        </IconButton>
                      )}
                    </Typography>
                  )}
                  {currentQuestion.type === "writing" || currentQuestion.type === "essay" ? (
                    <TextField
                      fullWidth
                      multiline
                      rows={currentQuestion.type === "essay" ? 15 : 8}
                      placeholder={
                        currentQuestion.type === "essay"
                          ? "Write your essay here (minimum 300 words recommended)..."
